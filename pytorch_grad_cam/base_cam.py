...
class BaseCAM:
    def __init__(self,
        ...
        self.activations_and_grads = ActivationsAndGradients(
            self.model, target_layers, reshape_transform, keep_on_device)
    ...
    def get_cam_image(self,
        ...
        if torch.is_tensor(activations):
            ...
            if eigen_smooth:
                return get_2d_projection(
                    weighted_activations.detach().cpu().numpy()
                )
    ...
    def forward(self,
        ...
        if self.uses_gradients:
            ...
            if isinstance(input_tensor, list):
                # outputs[0] 是 batch logits；每个 target 对应 batch 中的一个样本。
                # 原实现将 outputs 元组本身参与 zip，只适用于 batch=1。
                loss = sum(
                    target(output)
                    for target, output in zip(targets, outputs[0])
                )
            else:
    ...
    def compute_cam_per_layer(
        ...
        activations_list = [
            a if a.is_cuda else a.numpy()
            for a in self.activations_and_grads.activations
        ]
        grads_list = [
            g if g.is_cuda else g.numpy()
        ...
        for i in range(len(self.target_layers)):
            ...
            if torch.is_tensor(cam):
                cam = torch.relu(cam.float())
                cam = self.scale_cam_tensor(cam, target_size)
    ...
    @staticmethod
    def scale_cam_tensor(cam: torch.Tensor, target_size=None) -> torch.Tensor:
        cam = cam - cam.amin(dim=(1, 2), keepdim=True)
        cam = cam / (cam.amax(dim=(1, 2), keepdim=True) + 1e-7)
    ...
    def aggregate_multi_layers(self, cam_per_target_layer: np.ndarray) -> np.ndarray:
        if torch.is_tensor(cam_per_target_layer[0]):
            cam_per_target_layer = torch.cat(cam_per_target_layer, dim=1)
            result = torch.relu(cam_per_target_layer).mean(dim=1)
            result = self.scale_cam_tensor(result)
