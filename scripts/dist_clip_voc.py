...
parser.add_argument("--batch_size", default=None, type=int,
                    help="override samples_per_gpu in config")
...
def setup_seed(seed):
    ...
    np.random.seed(seed)
    random.seed(seed)
    # 输入裁剪尺寸固定，启用 cuDNN 自动选取高性能卷积算法。
    torch.backends.cudnn.deterministic = False
    torch.backends.cudnn.benchmark = True
    # RTX A6000 支持 TF32，可加速 FP32 卷积和矩阵乘法。
...
def train(cfg):
    ...
    # 掩码在整个训练过程中保持不变，提前传到 GPU，避免每轮重复转换。
    attn_mask = torch.from_numpy(
        get_mask_by_radius(h=mask_size, w=mask_size, radius=args.radius)
    ...
    # 在 GPU 上累计损失，只在日志间隔到达时同步一次到 CPU。
    # 这样可避免每轮调用 .item() 导致 CUDA 流被强制同步。
    loss_sums = torch.zeros(2, device='cuda')


    # 使用 tqdm 显示训练进度、预计剩余时间及当前损失。
    train_progress = tqdm(
    ...
    for n_iter in train_progress:
        ...
        inputs = inputs.cuda(non_blocking=True)
        segs, cam, attn_pred = WeCLIP_model(inputs, img_name, cls_labels=cls_labels)
        ...
        loss_sums += torch.stack((seg_loss.detach(), attn_loss.detach()))
        ...
        if (n_iter + 1) % cfg.train.log_iters == 0:
            ...
            avg_seg_loss, avg_attn_loss = (
                loss_sums / cfg.train.log_iters
            ).tolist()
            loss_sums.zero_()

            logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e;, pseudo_seg_loss: %.4f, attn_loss: %.4f, pseudo_seg_mAcc: %.4f"%(n_iter+1, delta, eta, cur_lr, avg_seg_loss, avg_attn_loss, seg_mAcc))

            writer.add_scalars('train/loss',  {"seg_loss": avg_seg_loss, "attn_loss": avg_attn_loss}, global_step=n_iter)

            # tqdm 指标仅按日志间隔更新，避免每轮读取 GPU 标量造成同步等待。
            train_progress.set_postfix(
                seg_loss=f'{avg_seg_loss:.4f}',
                attn_loss=f'{avg_attn_loss:.4f}',
                lr=f'{cur_lr:.3e}',
                refresh=False,
            )
