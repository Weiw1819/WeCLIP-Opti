...
class CLIP(nn.Module):
    ...
    def forward_last_layer(self, image_features, text_features, text_mask=None):
        ...
        if text_features.ndim == 2:
            # 原单样本路径：所有图像共享同一组文本类别。
            text_features = text_features / text_features.norm(dim=1, keepdim=True)
            logits_per_image = logit_scale * image_features @ text_features.t()
        else:
            # 批量 Grad-CAM 路径：每个图像-类别对拥有独立的候选文本集合。
            ...
            if text_mask is not None:
                logits_per_image = logits_per_image.masked_fill(
                    ~text_mask, torch.finfo(logits_per_image.dtype).min
                )
